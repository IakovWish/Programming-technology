...
bool Field::isNewStepPossible(int& possible, Cell(&possib)[10][10])
{
    for (int i = 0; i < 10 && possible < 3; i++)
    {
        for (int j = 0; j < 10 && possible < 3; j++)
        {
            ...
            {
                if (possib[i][j] != Cell::POSSIB)
                {
                    possible++;
                }
                possib[i][j] = Cell::POSSIB;
                Cell cell2 = cell1 == Cell::O ? Cell::CATCH_X : Cell::X;
                setCell(i, j, cell2);
                if (isNewStepPossible(possible, possib))
                {
                    setCell(i, j, cell1);
                    return true;
                }
...
bool Field::isPossible(int x, int y)
{
    ...
    if ((x == 0 && y == 0 && cell != Cell::O && getCell(10 - 1, 10 - 1) != Cell::X) || (x == 10 - 1 && y == 10 - 1 && cell != Cell::O && getCell(0, 0) != Cell::X) )
    {
        return true;
    }

    Cell massiv[10][10] = { Cell::ZERO };
