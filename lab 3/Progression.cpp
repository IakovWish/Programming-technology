...
{
	ifstream fin(path); // открыли файл дл€ чтени€
	if (!fin.is_open())
	{
		cout << "‘айл не может быть открыт!" << endl;
	}
	else
	{
		...
		if (--cnt % 4 != 0)
		{
			cout << "ќй-Єй похоже с вашим файлом что-то не так!" << endl;
		}
		else
		{
			int size = cnt / 4;
			...
			cout << "¬ведите 1 если хотите изменить путь к файлу дл€ переменных из файла, если нет введите любую другую цифру" << endl;
			cin >> opt;

			if (opt == 1)
			{
				cout << "¬ведите путь или им€ файла" << endl;
				cin >> way;
			}
...
{
	cout << "¬ведите первого элемента последовательности" << endl;
	cin >> member1;
	cout << "¬ведите посто€нную разность или отношение" << endl;
	cin >> const_diff_att;
}