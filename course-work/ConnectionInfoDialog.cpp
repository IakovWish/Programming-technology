...
ConnectionInfoDialog::ConnectionInfoDialog(QWidget* parent): QDialog(parent)
{
    ...
    serverCheckBox = new QCheckBox(this);
    ...
    layout->addWidget(serverLabel, 3, 2);
    layout->addWidget(serverCheckBox, 3, 3);
    ...
    prefLabel->setText(tr("Play with:"));
    serverLabel->setText(tr("Robot:"));
    ...
    connect(serverCheckBox, SIGNAL(toggled(bool)), this, SLOT(ServerGame()));
}
...
void ConnectionInfoDialog::ServerGame() // нажание на server checkbox
{
    ...
    {
        prefTextBox->clear();
        prefTextBox->setDisabled(true);
    }
    else
    {
        prefTextBox->setDisabled(false);
    }
}
...
void ConnectionInfoDialog::accept()
{
    ...
    server_check = serverCheckBox->isChecked();
