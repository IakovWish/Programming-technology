...
void Server::timerEvent(QTimerEvent* event)
{
    ...
    for (ClientsIterator cit = clients_.begin(); cit != clients_.end() && clients_.size() != 0; /* don't increment here */)
    {
        //Remove disconnected clients from list
        if (cit->status == Client::ClientStatus::DISCONNECTED)
        {
            if (freeClient == cit && (cit->status_game == Client::ClientStatusGame::ONLINE || cit->status_game == Client::ClientStatusGame::ONLINE_PREF))
            {
        ...
        if (cit->status != Client::ClientStatus::READY)
        {
            cit++;
            continue;
        }

        if (cit->status == Client::ClientStatus::READY && cit->status_game == Client::ClientStatusGame::OFFLINE)
        {
        ...
        if (freeClient != clients_.end() && freeClient != cit && freeClient->status == Client::ClientStatus::READY && cit->status_game == Client::ClientStatusGame::ONLINE)
        {
        ...
        if (cit->status_game == Client::ClientStatusGame::ONLINE_PREF && clients_.size() > 1)
        {
            ClientsIterator cit_pref = clients_.begin();
            while (cit_pref != clients_.end())
            {
                if ((cit_pref->status_game == Client::ClientStatusGame::ONLINE_PREF && cit_pref->status == Client::ClientStatus::READY && cit->prefLogin == cit_pref->login && cit_pref->prefLogin == cit->login && cit != cit_pref) || (cit->prefLogin == cit_pref->login && cit_pref->status_game == Client::ClientStatusGame::ONLINE && cit_pref->status == Client::ClientStatus::READY && cit != cit_pref))
                {
                    connectTwoClients(cit_pref, cit);
                    break;
                }
                cit_pref++;
            }
        ...
        if (cit->status_game != Client::ClientStatusGame::ONLINE_PREF)
        {
            freeClient = cit;
        }
        cit++;
...
{
    QRegExp rx(QString("mbclient:(\\d+):((\\w|\\d){%1,%2}):(.+):((\\w|\\d){0,%2}):").arg(LOGIN_LENGTH_MIN).arg(LOGIN_LENGTH_MAX));
    ...
    if ((client->status == Client::ClientStatus::AUTHORIZED) && client->login == login)
    {
        client->send("alreadyauth:");
        disconnectClient(client);
        return true;
    ...
    if (pref == "server")
    {
        client->status_game = Client::ClientStatusGame::OFFLINE;
    }
    else if (pref == "")
    {
        client->playingWith = clients_.end();
        client->status_game = Client::ClientStatusGame::ONLINE;
    }
    else
    {
        client->playingWith = clients_.end();
        client->prefLogin = pref;
        client->status_game = Client::ClientStatusGame::ONLINE_PREF;
    }
...
bool Server::stateRecieveField(const QString& cmd, ClientsIterator client)
{
    ...
    if (client->status == Client::ClientStatus::MAKING_STEP && client->status_game == Client::ClientStatusGame::OFFLINE)
    {
...
bool Server::stateRecieveSteps(const QString& cmd, ClientsIterator client)
{
    ...
    Field* field2 = client->field();

    if (client->status_game == Client::ClientStatusGame::ONLINE || client->status_game == Client::ClientStatusGame::ONLINE_PREF)
    {
    ...
    if (is3step)
    {
        ...
        if (client->status_game == Client::ClientStatusGame::ONLINE || client->status_game == Client::ClientStatusGame::ONLINE_PREF)
        {
            ...
            Field::Cell possib[10][10] = { Field::Cell::ZERO };

            for (int i = 0; i < 10; i++)
            {
                for (int j = 0; j < 10; j++)
                {
                    possib[i][j] = Field::Cell::ZERO;
                }
            }

            if (!field2->isNewStepPossible(possible, possib))
            {
        ...
        else
        {
            ...
            if (!field3->isNewStepPossible(possible, possib))
            {
            ...
            if (!field1->isNewStepPossible(possible, possib))
            {
...
void Server::disconnectClient(ClientsIterator client)
{
    ...
    {
        // TODO: understand what is written above
    }
