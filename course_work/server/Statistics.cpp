...
void Statistics::save( const QString& fname )
{
    ...
    for(QMap<QString, PlayerStats>::iterator i = data.begin(); i != data.end(); i++)
    {
        sortStruct tmp;
        tmp.player = i.key();
    ...
    for(QMap<quint32,sortStruct>::iterator i=sortedStat.end()-1; i!=sortedStat.begin(); i--)
    {
        out << QString( "%1:%2:%3:\n" ).arg( i->player ).arg( i.key() ).arg( i->roundsLost );
    }
    out << QString( "%1:%2:%3:\n" ).arg( sortedStat.begin()->player ).arg( sortedStat.begin().key() ).arg( sortedStat.begin()->roundsLost );
    out << flush;
    file.close();
}
